  ...
  return (
    <Popover>
      <PopoverTrigger className="relative flex size-10 items-center justify-center rounded-lg">
        ...
        { count > 0 && (
          <div className="absolute right-2 top-2 z-20 rounded-full size-2 bg-blue-500" />
        )}
      </PopoverTrigger>
        ...
        <LiveblocksUIConfig overrides={{
          ...
          <InboxNotificationList>
            ...
            {unreadNotifications.length > 0 && unreadNotifications.map((notification) => (
              <InboxNotification key={notification.id} inboxNotification={notification} className='bg-dark-200 text-white' href={`/documents/${notification.roomId}`} showActions={false} kinds={{
                thread: (props) => (
